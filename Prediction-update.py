# -*- coding: utf-8 -*-
"""
Created on Sat Jul 18 17:47:39 2026
...
# 2. 设置迭代参数 ----
n_repeats = 100  # 重复次数，建议至少 50-100 次以获得稳定的中位数
...
for i, (train_index, test_index) in enumerate(rkf.split(X)):
    ...
    model = lgb.LGBMRegressor(
        ...
        learning_rate=0.05,
        n_estimators=100,  # 建议先通过 cv 确定一个合理的固定值
    ...
    # 4. 可选：每隔一定次数计算 SHAP (因为 SHAP 计算较慢)
    # 如果样本量小，可以每一轮都算，最后取平均
...
# 3. 导出每轮重复的 Pearson r 结果 (pearson_per_repeat)
...
# 1. 数据准备 (保持原样)
dff = pd.read_stata('D:/真菌分析/data_pre/2015_mmsepredict_conti.dta') #---CHNS 前瞻性预测+all features---
dff.set_index('SampleID', inplace=True)
data = dff.replace('.', np.nan)
categorical_cols = ['region'] 
data = pd.get_dummies(data, columns=categorical_cols)
...
# 2. 设置参数：平衡严谨性与速度
n_splits = 10
n_permutations = 100 # 减少为 100 次置换
kf = KFold(n_splits=n_splits, shuffle=True, random_state=2026) # 单次 10 折

def run_fast_evaluation(X_input, y_input):
    """单次 10 折交叉验证，速度提升 10-50 倍"""
    r_list = []
